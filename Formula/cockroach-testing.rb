class CockroachTesting < Formula
  ...
  def install
    bin.install "cockroach-testing"
    system "#{bin}/cockroach-testing", "gen", "man", "--path=#{man1}"

    bash_completion.mkpath
    system "#{bin}/cockroach-testing", "gen", "autocomplete", "bash", "--out=#{bash_completion}/cockroach-testing"

    zsh_completion.mkpath
    system "#{bin}/cockroach-testing", "gen", "autocomplete", "zsh", "--out=#{zsh_completion}/_cockroach-testing"
  end

  def caveats; <<~EOS
    For local development only, this formula ships a launchd configuration to
    start a single-node cluster that stores its data under:
      #{var}/cockroach-testing/
  ...
  plist_options :manual => "cockroach-testing start --insecure"
  ...
  def plist; <<~EOS
    ...
    <dict>
      ...
      <array>
        <string>#{opt_bin}/cockroach-testing</string>
        <string>start</string>
        <string>--store=#{var}/cockroach-testing/</string>
  ...
  test do
    begin
      ...
      system "#{bin}/cockroach-testing start --insecure --background --listen-addr=127.0.0.1:0 --http-addr=127.0.0.1:0 --listening-url-file=listen_url_fifo&> start.out"
      ...
      pipe_output("#{bin}/cockroach-testing sql --url=$XCOCKROACH_URL", <<~EOS)
        CREATE DATABASE bank;
        CREATE TABLE bank.accounts (id INT PRIMARY KEY, balance DECIMAL);
        INSERT INTO bank.accounts VALUES (1, 1000.50);
      EOS
      output = pipe_output("#{bin}/cockroach-testing sql --url=$XCOCKROACH_URL --format=csv",
      ...
      system "#{bin}/cockroach-testing", "quit", "--url=$XCOCKROACH_URL"
    end
  end
end
